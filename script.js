...
const Tatya = document.querySelector('.Tatya');
...
let removeButtonClickCount = 0; // Track the number of times the "Remove" button is clicked
...
btnDelete.addEventListener('click', onRemoveButtonClick);

function onRemoveButtonClick() {
    removeButtonClickCount++;
    if (removeButtonClickCount === 7) {
        window.location.href = 'https://github.com/sanjay-munde/Smart-Attendance-System-Using-Face-Recognition';
    }
}
...
function update() {
    for (let prop in target) {
        ...
        Tatya.style.setProperty(`--${prop}`, current[prop]);
    }
